...
Plugins extend the agent with lifecycle hooks, system prompt contributions, and slash commands. Each plugin is a directory under `plugins/` (built-in) or `~/.agent/plugins/` (user).
...
├── plugins/
│   └── evolution/
│       ├── __init__.py             # EvolutionPlugin
│       ├── plugin.json             # Plugin manifest
│       ├── detector.py             # CorrectionDetector
│       └── rules.py                # RuleStore + BehaviorRule
├── skills/
│   └── skill-manager/
│       ├── SKILL.md                # Skill management instructions
│       └── scripts/
│           ├── init_skill.py       # Scaffold new skill bundles
