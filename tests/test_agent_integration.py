...
def test_team_run_executes_members_and_requires_structured_quality(monkeypatch):
    import agent as agent_module
    ...
    async def run_team():
        ...
        async def fake_send_message(self, ctx, user_message, stream_callback=None):
            if "fast reviewer" in user_message:
                fast_started.set()
                await slow_started.wait()
                content = json.dumps(
                    {
                        "findings": ["fast finding"],
                        "evidence": ["fast evidence"],
                        "confidence": 0.8,
                        "risks": ["fast risk"],
                    }
                )
            elif "slow reviewer" in user_message:
                slow_started.set()
                await fast_started.wait()
                content = json.dumps(
                    {
                        "findings": ["slow finding"],
            ...
            return agent_module.AgentResult(agent_id=ctx.agent_id, content=content)
    ...
    payload = asyncio.run(run_team())

    assert payload["ok"] is True
    assert payload["goal"] == "review orchestration"
    assert payload["summary"]["completed"] == 2
    assert payload["summary"]["failed"] == 0
    assert payload["summary"]["invalid"] == 0
    assert [member["name"] for member in payload["members"]] == ["fast", "slow"]
    assert payload["members"][0]["result"]["findings"] == ["fast finding"]
    assert payload["members"][1]["result"]["evidence"] == ["slow evidence"]


def test_team_run_honors_concurrency_limit(monkeypatch):
    import agent as agent_module
...
def test_team_run_marks_unstructured_member_result_invalid(monkeypatch):
    ...
    async def fake_send_message(self, ctx, user_message, stream_callback=None):
        return agent_module.AgentResult(agent_id=ctx.agent_id, content="plain text")
    ...
    assert payload["members"][0]["ok"] is False
    assert payload["members"][0]["quality_error"]
    assert "findings" in payload["members"][0]["quality_error"]
